...
class Openvopen_illegalVerts:
    def __init__(self, graph, start, goal, length_prefix_set):
        ...
        self.graph = graph
        self.length_prefix_set = length_prefix_set

        # cells[i]['F'][prefix][v] is a list of states with head=i, in forward direction, with prefix, which contain v (not head)
        # cells[i]['B'][prefix][v] is a list of states with head=i, in backward direction, with prefix, which contain v (not head)
        self.cells = [
            {
                'F': {},
                'B': {}
    ...
    def insert_state(self, state, is_f, stats=None):
        ...
        cell_dir = self.cells[cell_index][direction]

        is_new_prefix = prefix not in cell_dir
        if is_new_prefix:
            cell_dir[prefix] = [[] for _ in range(self.n)]
            if stats is not None:
                stats["num_of_prefix_sets"][direction] += 1

        for v in state_path[:-1]:  # Exclude head        
            bucket = cell_dir[prefix][v]
            bucket.append(state)
            idx = len(bucket) - 1
            self._loc[state] = (cell_index, direction, prefix, g_value, idx)

        self.num_states[direction] += 1
        self.counter += 1

        if stats is not None:
            k = stats["num_of_prefix_sets"][direction]
            stats["prefix_set_mean_size"][direction] = (self.num_states[direction] / k) if k else 0.0
    ...
    def remove_state(self, state, is_f):
        ...
        cell_index, target_recorded, prefix, g_value, idx = info
        ...
        bucket = self.cells[cell_index][target_recorded][prefix][g_value]
