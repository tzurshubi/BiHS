...
class Openvopen_prefixSet:
    def __init__(self, graph, start, goal, length_prefix_set):
        ...
        # cells[i]['F'][prefix][g] is a list of states with head=i, in forward direction, with prefix, and g=g (unsorted bucket)
        # cells[i]['B'][prefix][g] is a list of states with head=i, in backward direction, with prefix, and g=g (unsorted bucket)
        self.cells = [
            {
                'F': {},
                'B': {}
    ...
    def find_all_non_overlapping_paths(self, state, is_f, best_path_length, f_max, snake=False):
        ...
        for prefix in opp_struct:
            ...
            for g_candidate in range(self.n - 1, -1, -1):
                bucket = opp_struct[prefix][g_candidate]
                if not bucket:
                    continue
