...
def BiXIDA(graph, start, goal, heuristic_name, snake, args):
    logger = args.logger 
    N = max(graph.nodes)
    V = len(graph.nodes)
    ...
    # --- IDA* Specific Variables ---
    # Calculate the initial upper bound threshold
    initial_h = V
    if heuristic_name:
    ...
    def exp_n_check_states(state_F, state_B, h_graph, expand_F_turn=True):
        ...
        # logger(f"Exp {stats['expansions']}: Expanding state_F {state_F.materialize_path()} and state_B {state_B.materialize_path()}")
        ...
        for h_val, leaf_F, leaf_B, leaf_h_graph in leaves:
            ...
            # logger(f"Checking leaf_F {leaf_F.materialize_path()} and leaf_B {leaf_B.materialize_path()} with h={h_val}, f={current_f_value}, g_F={leaf_F.g}, g_B={leaf_B.g}")
            ...
            if current_f_value < threshold: 
                ...
                if current_f_value > next_threshold:
                    next_threshold = current_f_value
                continue # Note: 'continue' instead of 'break' if leaves aren't strictly monotonic, though 'break' is fine if perfectly sorted descending.
            ...
            # Check if we just found a path that meets our exact threshold
    ...
    while threshold >= 0 and not target_found:
        # logger(f"Starting IDA* iteration with threshold: {threshold}")
        next_threshold = -1
        
        # Reset FNV for the new iteration (crucial for IDA*)
        if args.bsd:
        ...
        # Check if the iteration found the target
        if len(global_longest_path) - 1 == threshold:
            # logger(f"Optimal path of length {threshold} verified.")
            break
            
        # If we didn't find a path of length `threshold`, lower the threshold for the next loop
