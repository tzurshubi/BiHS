...
def BiXDFBnB_strict_dovetailing(graph, start, goal, heuristic_name, snake, args):
    k_step_dict = {4:8, 2:100}
    logger = args.logger
    stats = args.stats 
    N = max(graph.nodes)
    V = len(graph.nodes)
    ...
    def exp_n_check_states(state_F, state_B, h_graph, expand_F_turn=True):
        ...
        # --- DOVETAILING: Yield control to simulate 1 quantum of computational effort ---
        yield 
        ...
        # Group by heuristic value to identify ties
        for h_val, group in groupby(leaves, key=lambda item: item[0]):
            group = list(group)
            valid_group = []
            
            # Evaluate bounds and BSD dynamically right before execution
            for _, leaf_F, leaf_B, leaf_h_graph in group:
                if args.bsd:
                ...
                # Because leaves are sorted and all items in leaves share the same combined g-depth, 
                # failing the bound here mathematically guarantees all subsequent leaves will also fail.
                if leaf_F.g + h_val + leaf_B.g <= len(global_longest_path) - 1: 
                    stats["violations"]["heuristic"][state_F.g] += 1
                    return # Prunes this group and entirely exits the generator for this parent
                    
                if args.bsd: FNV[double_state_key] = leaf_F.g + leaf_B.g
                valid_group.append((leaf_F, leaf_B, leaf_h_graph))
            ...
            if len(valid_group) == 1:
                # Single branch: Standard depth-first recursion
                leaf_F, leaf_B, leaf_h_graph = valid_group[0]
                yield from exp_n_check_states(leaf_F, leaf_B, leaf_h_graph, next_turn)
            else:
                # Tie detected: Interleave the search (Dovetailing)
                active_gens = [exp_n_check_states(lf, lb, lhg, next_turn) for lf, lb, lhg in valid_group]
                
                while active_gens:
                    next_active = []
                    for gen in active_gens:
                        try:
                            next(gen)  # Execute exactly 1 node expansion from this tied branch
                            yield      # Bubble the pause up to maintain global round-robin interleaving
                            next_active.append(gen)
                        except StopIteration:
                            pass       # The branch reached a dead end or was pruned by bounds
    ...
    if is_valid and should_continue:
        # Spin up and consume the top-level generator to run the interleaved search
