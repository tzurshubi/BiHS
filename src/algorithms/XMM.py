...
def XMM(graph, start, goal, heuristic_name, snake, args):
    # # For Plotting h
    # mis_smaller_flag = []
    # expansions_list = []
    # h_MIS = []
    ...
    while len(OPEN_F) > 0 or len(OPEN_B) > 0:
        ...
        # XMM_full (XMM): g cutoff. if g > f_max/2 don't expand it, but keep it in OPENvOPEN for checking collision with the other side
        if args.algo == "cutoff" or args.algo == "XMM":
            if (D=='F' and current_state.g > f_value/2 - 1) or (D=='B' and current_state.g > (f_value - 1)/2): 
                OPEN_D.pop()
                stats["moved_OPEN_to_AUXOPEN"] += 1
        ...
        for successor in successors:
            ...
            # XMM_light + PathMin
            if args.algo == "light" or args.algo == "XMM":
