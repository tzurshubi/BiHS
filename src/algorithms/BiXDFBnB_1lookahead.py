...
def BiXDFBnB_1lookahead(graph, start, goal, heuristic_name, snake, args):
    logger = args.logger 
    N = max(graph.nodes)
    V = len(graph.nodes)
    ...
    def exp_n_check_states(state_F, state_B, h_graph):
        ...
        if heuristic_name:
            for succ_F in state_F_successors:
                ...
                F_successors_with_h.append((h_val, succ_F))
            F_successors_with_h.sort(key=lambda item: item[0], reverse=True)
        else:
            for succ_F in state_F_successors:
                F_successors_with_h.append((V, succ_F))
        max_h_val_succ_F = F_successors_with_h[0][0] if F_successors_with_h else -1
        avg_h_val_succ_F = sum(h for h, _ in F_successors_with_h) / len(F_successors_with_h) if F_successors_with_h else -1

        # 1-step lookahead (expand B)
        ...
        if heuristic_name:
            for succ_B in state_B_successors:
                ...
                B_successors_with_h.append((h_val, succ_B))
            B_successors_with_h.sort(key=lambda item: item[0], reverse=True)
        else:
            for succ_B in state_B_successors:
                B_successors_with_h.append((V, succ_B))
        max_h_val_succ_B = B_successors_with_h[0][0] if B_successors_with_h else -1
        avg_h_val_succ_B = sum(h for h, _ in B_successors_with_h) / len(B_successors_with_h) if B_successors_with_h else -1

        # Determine which frontier to expand
        expand_F = True
        if max_h_val_succ_B > max_h_val_succ_F or (max_h_val_succ_B == max_h_val_succ_F and avg_h_val_succ_B > avg_h_val_succ_F):
            expand_F = False
